...
public class Balance extends JFrame implements ActionListener {
    ...
    private JTextField AccountNumberField,PinNumberField,BalanceField;
	...
	String filePath = ".\\All_Text_File\\Balance.txt";
    ...
    public Balance() {
        ...
        providerlable.setBounds(15, 100, 120, 30);
        providerlable.setFont(fontstyle);
		providerlable.setForeground(Color.black);

        String[] country1 = {"Bkash", "Nagad", "Upay", "Rocket", "VISA", "Master Card", "Bank Asia", "Dhaka Bank", "BRAC", "Prime Bank", "Sonali Bank", "Standard Bank"};
        provider = new JComboBox<>(country1);
        provider.setBounds(15, 132, 200, 30);
		
		JLabel name = new JLabel("Name:");
        name.setBounds(237, 100, 150, 30);
        name.setFont(fontstyle);
		name.setForeground(Color.black);

        nameField = new JTextField();
        nameField.setBounds(237, 132, 200, 30);
        nameField.setFont(fontstyle);
        nameField.setToolTipText("Enter Name");

        JLabel AccountNumber = new JLabel("Account Number:");
        AccountNumber.setBounds(458, 100, 150, 30);
        AccountNumber.setFont(fontstyle);
		AccountNumber.setForeground(Color.black);

        AccountNumberField = new JTextField();
        AccountNumberField.setBounds(458, 132, 200, 30);
        AccountNumberField.setFont(fontstyle);
        AccountNumberField.setToolTipText("Enter Account Number");
		
		JLabel PinNumber = new JLabel("Pin Number:");
        PinNumber.setBounds(679, 100, 150, 30);
        PinNumber.setFont(fontstyle);
		PinNumber.setForeground(Color.black);

        PinNumberField = new JTextField();
        PinNumberField.setBounds(679, 132, 200, 30);
        PinNumberField.setFont(fontstyle);
        PinNumberField.setToolTipText("Enter Pin Number");

        JLabel BalanceLabel = new JLabel("Balance:");
        BalanceLabel.setBounds(895, 100, 150, 30);
        BalanceLabel.setFont(fontstyle);
		BalanceLabel.setForeground(Color.black);

        BalanceField = new JTextField();
        BalanceField.setBounds(895, 132, 200, 30);
        BalanceField.setFont(fontstyle);
        BalanceField.setToolTipText("Enter Balance");

        
        
		

        String[] column = {"Service Provider", "Name", "Account Number", "Pin", "Balance"};
        ...
        f.add(providerlable);
        f.add(AccountNumber);
        f.add(AccountNumberField);
        f.add(name);
        f.add(nameField);
		f.add(BalanceLabel);
        f.add(BalanceField);
		f.add(PinNumber);
        f.add(PinNumberField);
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == submit) {
            ...
            String getAccountNumber = AccountNumberField.getText();
            String getBalance = BalanceField.getText();
            String getPinNumber = PinNumberField.getText();
            
			

            if (getname.equals("") || getAccountNumber.equals("") || getBalance.equals("")|| getPinNumber.equals("")) {
                JOptionPane.showMessageDialog(null, "One or more fields are empty");
            } 
			else {
				String[] data = {getprovider, getname, getAccountNumber, getPinNumber, getBalance};
                tblModel.addRow(data);
				
                try {
                    FileWriter fw = new FileWriter(".\\All_Text_File\\Balance.txt");
        ...
        if (selectedRow != -1) {
            ...
            try {
                FileWriter fw1 = new FileWriter(".\\All_Text_File\\Balance.txt");
